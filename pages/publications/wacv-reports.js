...
function Photosalbum(props) {
  ...
  const fetchPhotos = async (currentPage) => {
    const res = await fetch(
      `${API_URL}/api/wacv-reports?populate[0]=Report&populate[1]=Report.cover&populate[2]=Report.file&sort=rannk:asc&pagination[page]=1&pagination[pageSize]=${limit}`
    );
...
export async function getStaticProps() {
  const res = await fetch(
    `${API_URL}/api/wacv-reports?populate[0]=Report&populate[1]=Report.cover&populate[2]=Report.file&sort=rannk:asc&pagination[page]=1&pagination[pageSize]=10`
  );
